...
class ImageManager extends Injectable
{
     ...
     */
    public static $allowedImageExtensions = ['png', 'jpg', 'jpeg'];
    public static $recipe_image_dir = 'r';
    public static $recipe_step_image_dir = 'rs';
    public static $category_image_dir = 'c';
...
//
    public static function resize($path, $type = '')
    {
        if (!$type) {
        ...
        foreach ($image_types as $image_type) {
            ImageManager::resizeForImageType($path, $image_type);
        }
        return true;
    }

    public static function resizeForImageType($path, ImageType $image_type)
    {
        $image_imagick = new Imagick($path);
...
//        $canvas->watermark($image_imagick, $x, $y, 100);
        $image_imagick->save(self::getPathWithType($path, $image_type->getType()), 90);
    }
...
//    }
    ...
    /**
     * Return path with image's type
     * @param $path
     * @param string $type
     * @return string
     */
    public static function getPathWithType($path, $type = '')
    {
        $base_path = dirname($path);
        $filename = basename($path);
        $path_info = pathinfo($filename);
        return $base_path . DIRECTORY_SEPARATOR . $path_info['filename'] . '-' . $type . '.' . $path_info['extension'];
    }
